...
inline void InitSubAllocator()
{
    ...
    for (i=0,k=1;i < (int)N1           ;i++,k += 1)    Indx2Units[i]=k;
    for (    k++;i < (int)(N1+N2)      ;i++,k += 2)    Indx2Units[i]=k;
    for (    k++;i < (int)(N1+N2+N3)   ;i++,k += 3)    Indx2Units[i]=k;
    for (    k++;i < (int)(N1+N2+N3+N4);i++,k += 4)    Indx2Units[i]=k;
...
inline void GlueFreeBlocks()
{
    ...
    for (i=0, s0.next=s0.prev=&s0;i < (int)N_INDEXES;i++)
